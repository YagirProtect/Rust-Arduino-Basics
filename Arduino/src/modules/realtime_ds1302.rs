    ...
    RST: PinOps,
{
    ...
    fn write_read_command(&mut self, mut cmd: u8) {
        ...
        for bit_index in 0..8 {
            ...
            // ВАЖНО:
            // перед последним falling edge отпускаем DAT,
            // потому что на этом спаде DS1302 уже начинает выдавать первый бит ответа.
            if bit_index == 7 {
                self.dat_to_input();
            }
    ...
    fn read_byte(&mut self) -> u8 {
        ...
        for i in 0..8 {
            ...
            // Первый бит уже валиден после команды чтения,
            // поэтому сначала читаем, потом даём следующий такт.
            if dat.is_high() {
                value |= 1 << i;
            }
